...
class FractionStarsModel {
  ...
  generateRandomProblem() {
    // Limit denominator to max 7 as specified
    this.denominator = Math.floor(Math.random() * 6) + 2; // 2 to 7
    this.numerator = Math.floor(Math.random() * (this.denominator - 1)) + 1;

    // Ensure total is divisible by denominator - make sure it's between 24-42
    const possibleTotals = [10, 18, 40, 30, , 35, 36, 40, 42];
    // let validTotals = possibleTotals.filter(num => num % this.denominator === 0);

    let validTotals = possibleTotals.filter(
      (num) => num % this.denominator === 0 && num / this.denominator < 10
    );

    // // If no exact match, find multiples that work
    // if (validTotals.length === 0) {
    //   for (let i = 24; i <= 42; i += this.denominator) {
    //     validTotals.push(i);
    //   }
    // }

    // Pick a random valid total
    this.total = validTotals[Math.floor(Math.random() * validTotals.length)];
...
class FractionStarsView {
  constructor(controller) {
    ...
    this.groupSelectedColor = "rgba(255, 165, 0, 0.9)";
  ...
  bindEvents() {
    document.getElementById("groupSlider").addEventListener("input", () => {
      const value = parseInt(document.getElementById("groupSlider").value);
  ...
  drawFraction(p, numerator, denominator, x, y, fontSize) {
    ...
    const fractionWidth =
      Math.max(numWidth, denomWidth) + this.fractionPadding * 2;
  ...
  drawGroups(p) {
    ...
    // Highlight the first group when showSelectInstruction is true
    // if (this.showSelectInstruction && i === 0) {
    //   p.fill('rgba(247, 156, 243, 0.2)'); // Light orange highlight for the first group
    // } else {
    //   p.noFill();
    // }
    if (this.showSelectInstruction && i === 0) {
  // Use frameCount or a toggle based on time to alternate visibility
  const blinkSpeed = 30; // Adjust this for faster/slower blink (frames)
  if ((p.frameCount % blinkSpeed) < blinkSpeed / 2) {
    p.fill('rgba(247, 156, 243, 0.2)'); // Light highlight when "visible"
  } else {
    p.noFill(); // Invisible (hide)
  }
} else {
  p.noFill();
}
      ...
      if (model.groups === 1) {
        ...
        const total = model.total; // e.g. 30
        const denominator = model.denominator; // e.g. 5
        let cols, rows;

        if ((total / denominator ) > 5) {
          cols = Math.floor(total / denominator);
          rows = denominator;
        } else {
          rows = Math.floor(total / denominator);
          cols = denominator;
        }
    
        const gridSize = this.starSize + this.starMargin * 2;
        for (let j = 0; j < group.stars; j++) {
          // horizontal grouping (row by row)
          const col = j % cols;
          const row = Math.floor(j / cols);

          // For vertical grouping (column by column), use:
          // const row = j % rows;
          // const col = Math.floor(j / rows);
          // the starX and starY are the center of the star
          
          const starX =
            group.x + this.groupPadding + row * gridSize + gridSize / 2;
          const starY =
            group.y + this.groupPadding + col * gridSize + gridSize / 2;
    ...
    //   let cols, rows;
    //   if ((total % denominator) > 5) {
    //     rows = total / denominator;
    //     cols = 5;
    //   } else {
    //     cols =total / denominator;
    //     rows = 5;
    //   }
    //   const gridSize = this.starSize + this.starMargin * 2;

    //   for (let k = 0; k < group.stars; k++) {
    //     const col = k % cols;
    //     const row = Math.floor(k / cols);
    //     const starX = group.x + this.groupPadding + col * gridSize + gridSize / 2;
    //     const starY = group.y + this.groupPadding + row * gridSize + gridSize / 2;
    //     p.text("★", starX, starY);
    //   }
  ...
  drawExtraStars(p) {
    if (this.extraStarsData.length > 0 && this.groupsData.length > 0) {
      ...
      let maxY = 0;
      for (let i = 0; i < this.groupsData.length; i++) {
        const groupBottom = this.groupsData[i].y + this.groupsData[i].height;
        if (groupBottom > maxY) {
          maxY = groupBottom;
        }
      }
  ...
  updateDisplay() {
    ...
    this.showSelectInstruction =
      model.isCorrectGroupCount() && model.selectedGroups.length === 0;
  ...
  //     const groupHeight = rows * gridSize + this.starMargin * 2;
  ...
  // }
   ...
   calculateGroupsLayout() {
    ...
    if (model.groups === 1) {
      ...
      let cols, rows;
      // Using the new specified logic for groups === 1
      if (total / denominator > 5) {
        rows = Math.floor(total / denominator);
        cols = denominator;
      } else {
      ...
      const gridSizeX = this.starSize + this.starMargin *2;
      const gridSizeY = this.starSize + this.starMargin *2;
      
      // Calculate group dimensions based on grid
      const groupWidth = cols * gridSizeX + this.groupPadding * 2;
      const groupHeight = rows * gridSizeY + this.groupPadding * 2;
      ...
      this.groupsData.push({
        ...
        height: groupHeight,
        stars: starsPerGroup,
        cols: cols, // Store column count for drawing
        rows: rows  // Store row count for drawing
      });
    } else {
      // For multiple groups
      const colsPerGroup = starsPerGroup > 9 ? 2 : 1;
      ...
      // Handle any remaining extra stars
      if (extraStars > 0) {
        const extraStarY =
          this.headerHeight + 10 + rows * (groupHeight + 10) + 10;
        const extraStarStartX =
