document.addEventListener("DOMContentLoaded", () => {
  ...
  easy: [
    ...
    { word: "kook", clue: "An eccentric or strange person" },
...
function checkAnswer() {
  ...
  if (allCorrect) {
    ...
    score++;
    streak++;
    currentList[currentIndex].status = 'solved';
...
function renderProgress() {
  ...
  currentList.forEach((item, i) => {
    ...
    if (item.status === 'solved') dot.classList.add('solved');
    if (item.status === 'skipped') dot.classList.add('skipped');
    el.appendChild(dot);
  });
}
