...
class PointsModel {
    ...
    calculateDistance() {
        if (this.selectedPoints.length !== 2) return null;
...
class GraphView {
    ...
    drawTriangle(pointA, pointB) {
        ...
        this.triangleObjects.push(
            ...
            this.board.create('text', [
                (x1 + x2) / 2, (y1 + y2) / 2,
                () => `d = ${Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)).toFixed(2)}`
            ], { fontSize: 14 })
        );
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createDraggableLabels() {
    ...
    // Update text elements in JSXGraph to be directly on the sides
    const dText = board.create('text', [
      (xA + xB) / 2,  // Center of hypotenuse (x)
      (yA + yB) / 2,  // Center of hypotenuse (y)
      'd'
    ], {
      fontSize: 14,
      color: '#d63384',
      ...
      anchorX: 'middle',
      anchorY: 'middle',
      highlightStrokeColor: 'none', // Disable highlight
      highlightFillColor: 'none'    // Disable highlight
    });
    
    const xDiffText = board.create('text', [
      (xA + xC) / 2,  // Center of horizontal side (x)
      yA,             // Exactly on the horizontal line
      'x₂-x₁'
    ], {
      fontSize: 14,
      color: '#3b82f6',
    ...
    const yDiffText = board.create('text', [
      xC,             // Exactly on the vertical line
      (yC + yB) / 2,  // Center of vertical side
      'y₂-y₁'
    ], {
      fontSize: 14,
      color: '#3b82f6',
    ...
    triangleLabels = [dText, xDiffText, yDiffText];
    
    // Get pixel coordinates for draggable labels
    const coords = board.getCoordsTopLeftCorner();
    const scales = board.getBoundingBox();
    const boardWidth = board.canvasWidth;
    const boardHeight = board.canvasHeight;
    
    // Convert JSXGraph coordinates to pixel coordinates
    function toPixelX(x) {
      return coords[0] + (x - scales[0]) * (boardWidth / (scales[2] - scales[0]));
    }
    
    function toPixelY(y) {
      return coords[1] + (scales[1] - y) * (boardHeight / (scales[1] - scales[3]));
    }
    
    // Update the draggable HTML elements to match the text positions
