...
class LottieSVGAnimation {
    ...
    init() {
        ...
        this.animation.addEventListener('DOMLoaded', () => {
            ...
            // *** CHANGE: Set initial visual state to the start frame of segment-22 (Frame 100) ***
            // goToAndStop(frame, isFrame) immediately jumps to the frame and stops.
            // true for isFrame means the first argument is a frame number, not a time.
            this.animation.goToAndStop(120, true);
            console.log(`Lottie animation initially set to Frame 100 (Position 2 state).`);
            // ***********************************************************************************
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    'segment-11': [300, 600],       // Position 1 Idle Loop
    'segment-22': [120, 120],       // Position 2 Idle Loop
    'segment-33': [1400, 2000],   // Position 3 Idle Loop

    // Transition segments from Position 1
    'segment-12': [0, 120],     // Transition 1 -> 2
    'segment-13': [0, 200],   // Transition 1 -> 3

    // Transition segments from Position 2
    'segment-21': [120, 0],     // Transition 2 -> 1
    'segment-23': [120, 200],   // Transition 2 -> 3
    
    // Transition segments from Position 3
    'segment-31': [200, 0],   // Transition 3 -> 1
    'segment-32': [200, 120]    // Transition 3 -> 2
    };
     ...
     * with a 1500ms delay between each change.
     */
    ...
    // Explicitly set the starting text to '1 Day'
    ...
    function handleSegmentClick(event) {
        ...
        if (destinationId === currentPosition) {
            ...
            // Re-apply visual state for current position (default: Position 2)
            document.getElementById('beam-img1').style.display = 'none';
            document.getElementById('beam-img3').style.display = 'none';
