...
function initializeButtonReferences() {
  startBtn =
    document.querySelector("#Start")?.parentElement ||
    document.querySelector("#Group_5921");
  resetBtn =
    document.querySelector("#Reset")?.parentElement ||
    document.querySelector("#Group_5931");
  insightsBtn =
    document.querySelector("#Insights")?.closest("#Group_591") ||
    document.querySelector("#Group_591");
}

// playLottieAnimation and loadandPlayLottieAnimation are provided by lottieAll.js
...
function init() {
  ...
  setupTooltips();
  setInitialState();
  console.log("Lac Operon widget initialized", {
    startBtn,
    resetBtn,
    insightsBtn,
  });
...
function setupEventListeners() {
  ...
  if (svg) {
    svg.addEventListener("click", (e) => {
      ...
      if (
        parentGroup?.id === "Start" ||
        grandparentGroup?.id === "Group_5921"
      ) {
        e.stopPropagation();
        e.preventDefault();
        handleStartClick();
      ...
      if (
        ...
        grandparentGroup?.id === "Group_5931"
      ) {
        e.stopPropagation();
        e.preventDefault();
        handleResetClick();
      ...
      if (
        parentGroup?.id === "Insights" ||
        grandparentGroup?.id === "Group_591"
      ) {
        e.stopPropagation();
        e.preventDefault();
        handleInsightsClick();
      ...
      if (
        parentGroup?.id === "Group_595" ||
        grandparentGroup?.id === "Group_595"
      ) {
        e.stopPropagation();
        e.preventDefault();
        handleToggleClick();
...
function setInitialState() {
  ...
  const iText = document.getElementById(
    "Tap_Start_to_initiate_i_gene_s_expression_",
  );
...
async function handleStartClick() {
  ...
  // Play start animation, then absent animation

  playLottieAnimation("assets/JSON/absent.json");

  state.animationInProgress = false;
}

// Play present/absent and transition animations on toggle
function playInducerLottie(present) {
  if (present) {
    playLottieAnimation("./assets/JSON/present_opt.json");
  } else {
    playLottieAnimation("./assets/JSON/absent.json");
  }
}
...
function playLottieAnimation(path) {
  ...
  const parentEl = document.getElementById("lottie_animation");

  if (!containerEl || !parentEl) {
    console.warn("Lottie container elements not found");
    return;
  }
  ...
  try {
    lottieInstances = lottie.loadAnimation({
      ...
      renderer: "svg",
      loop: false,
      autoplay: true, // ← Changed to true (most reliable)
      path: path,
      rendererSettings: {
        preserveAspectRatio: "xMidYMid meet",
        progressiveLoad: false, // disable for better first-frame reliability
      },
    });

    // Fallback: force play after a short delay (helps in foreignObject + slow load cases)
    setTimeout(() => {
      if (lottieInstances && !lottieInstances.isPlaying) {
        console.log("Force playing Lottie:", path);
    ...
    console.log("Lottie animation started:", path);
  } catch (err) {
...
async function handleToggleClick() {
  ...
  // Play Lottie animation for toggle
  playInducerLottie(state.inducerPresent);
...
async function animateRepressorInactivation() {
  // Find repressor icon
  const repressorIcon = document.querySelector(
    'circle[data-element="repressor-icon"]',
  );
...
async function animateRNAPolymeraseTranscription() {
  ...
  await new Promise((resolve) => {
    ...
    const animate = () => {
      ...
      const circle = rnapGroup.querySelector(
        'circle[data-element="rna-polymerase-icon"]',
      );
...
async function handleInsightsClick() {
  ...
  if (Group_579)
    Group_579.addEventListener("click", () => {
...
function createNoEnzymeText() {
  const text = createDynamicText(960, 450, "No Enzyme Production", {
    fontSize: 26,
    fill: "#C90E0E",
    fontWeight: "bold",
    fontStyle: "italic",
...
function animateElement(element, animations) {
  return new Promise((resolve) => {
    ...
    animations.forEach((anim) => {
      const startValue =
        parseFloat(element.getAttribute(anim.attr)) || anim.from;
