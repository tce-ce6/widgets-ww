...
function markSlopeAsCompleted(title) {
  ...
  // ✅ Enable Home Reset Button after first completion
...
if (homeResetBtn) {
  homeResetBtn.addEventListener("click", () => {
    if (homeResetBtn.classList.contains("disabled")) return;

    // 🔁 Reset current activity safely
    resetActivity();
    ...
    // ❌ Remove completed state from ALL slopes
    document.querySelectorAll("#slope-list li").forEach((li) => {
      li.classList.remove("completed");
    });

    // 🧹 Hide all slope containers
    const slopeIds = [
      "gentle-slope-lines",
      "steep-slope-lines",
      "convex-slope-lines",
    ...
    // 🔄 Disable button again until next completion
    homeResetBtn.classList.add("disabled");
  });
}
