class MixedNumberModel {
    constructor() {
        ...
        this.blueAnimationProgress = 0;
        this.pinkAnimationProgress = 0;
        this.isAnimating = false;
        this.isSliderDragging = false;
        this.isAnimatingEquation = true; // Always true during dragging for parallel animation
        this.stepsHistory = [];
        ...
        this.pinkAnimationStarted = false;
        ...
        do {
            if (this.operation === 'subtraction') {
                do {
                    ...
                    num2 = Math.floor(Math.random() * 3) + 1;    // 1, 2, 3 (adjusted for valid subtraction)
            ...
            totalValue = this.operation === 'subtraction' ? value1 - value2 : value1 + value2;
            denomProduct = den1 * den2;
        } while (totalValue >= 10 || denomProduct >= 12 || totalValue < 0);
    ...
    calculateSteps() {
        ...
        if (sameDenominators) {
            ...
            lcm = this.lcm(first.den, second.den);
        ...
        this.steps = {
            ...
            equivalent: `${firstEquiv.whole} ${firstEquiv.num}/${firstEquiv.den} ${operatorSymbol} ${secondEquiv.whole} ${secondEquiv.num}/${secondEquiv.den}`,
    ...
    setCellIndex(index) {
        ...
        this.pinkAnimationStarted = this.blueAnimationComplete && this.currentCellIndex > this.blueCellsToAnimate;
        this.isAnimating = this.isSliderDragging;
        this.blueAnimationProgress = this.isSliderDragging ? this.currentCellIndex / this.blueCellsToAnimate : 1;
        this.pinkAnimationProgress = (this.isSliderDragging && this.pinkAnimationStarted) ? (this.currentCellIndex - this.blueCellsToAnimate) / this.pinkCellsToAnimate : 1;
    ...
    setSliderDragging(isDragging) {
        ...
        // if (!isDragging) {
        //     this.blueAnimationProgress = 1;
        //     if (this.pinkAnimationStarted) {
        //         this.pinkAnimationProgress = 1;
        //     }
        //     this.updateStepsHistory();
        // }
    }
 updateStepsHistory() {
        const stepTexts = [
            ...
            this.steps.added // Changed from this.steps.final to this.steps.added
        ];
        
        let mathStep = 0;
        if (this.blueAnimationComplete && this.blueAnimationProgress >= 1) {
            mathStep = 1;
        }
        if (this.currentCellIndex >= this.totalCellsToAnimate) { // Changed from pinkAnimationProgress >= 1
            mathStep = 2;
        }
        
        const previousLength = this.stepsHistory.length;
        this.stepsHistory = stepTexts.slice(0, mathStep + 1);
        return previousLength !== this.stepsHistory.length; // Return true if new step added
    }

    updateAnimation() {
        if (this.isAnimating) {
            if (!this.blueAnimationComplete) {
                this.blueAnimationProgress += 0.02;
                if (this.blueAnimationProgress >= 1) {
                    this.blueAnimationProgress = 1;
                }
            } else if (this.pinkAnimationStarted) {
                this.pinkAnimationProgress += 0.02;
                if (this.pinkAnimationProgress >= 1) {
                    this.pinkAnimationProgress = 1;
                }
            }
        }
...
class MixedNumberView {
    constructor(model, p5Instance) {
        ...
        this.cancelledCache = new Map();
        this.stepAlpha = new Map(); // Persist alpha for all steps
        this.animationStartFrame = null;
        this.animationDuration = 30; // 0.5 seconds at 60 FPS
        this.previousStepsLength = 0;
    }
     ...
     draw() {
        ...
        if (this.previousCellIndex !== this.model.currentCellIndex) {
            ...
            this.cancelledCache.clear();
            // Only reset animation if a new step is added
            const newStepAdded = this.model.updateStepsHistory();
            if (this.model.isAnimatingEquation && newStepAdded && this.model.stepsHistory.length > this.previousStepsLength) {
                this.animationStartFrame = this.p.frameCount;
                this.model.stepsHistory.forEach((step, index) => {
                    if (index >= this.previousStepsLength) {
                        this.stepAlpha.set(step, 0); // Animate only new steps
                    }
                });
                this.previousStepsLength = this.model.stepsHistory.length;
            }
        }
    ...
    setupBoxAnimations() {
        console.log("🚀 ~ MixedNumberView ~ setupBoxAnimations ~ this.model.currentCellIndex:", this.model.currentCellIndex);
        this.allBoxes = [];
        this.crossOutBoxes = [];
    ...
    setupFractionAnimations(destX, startY) {
        ...
        if (this.model.sameDenominators) {
            ...
            const rows = originalFirst;
            const cols = originalSecond;
    ...
    //         let alpha = 255;
    //         let progress = box.type === 'blue' ? this.model.blueAnimationProgress : this.model.pinkAnimationProgress;

    //         if (box.type === 'pink' && !this.model.pinkAnimationStarted) {
    //             continue; // Skip pink boxes until pink animation starts
    //         }

    //         if (this.model.currentCellIndex >= this.model.totalCellsToAnimate) {
    //             if (this.model.operation === 'addition') {
    //                 fillColor = this.p.color(30, 97, 38);
    //                 alpha = 255;
    ...
    //             const interpX = this.p.lerp(box.srcX, box.destX, progress);
    //             const interpY = this.p.lerp(box.srcY, box.destY, progress);
    //             this.p.fill(fillColor.levels[0], fillColor.levels[1], fillColor.levels[2], alpha);
    ...
    // }
     drawAnimatedCells() {
        for (let i = 0; i < this.allBoxes.length; i++) {
            ...
            // Check cache for isCancelled
            let isCancelled;
            if (this.cancelledCache.has(i)) {
                isCancelled = this.cancelledCache.get(i);
            } else {
                isCancelled = this.crossOutBoxes.some(crossBox =>
                    (crossBox.correspondingPinkIndex === i && box.type === 'pink') ||
                    (crossBox.x === box.destX && crossBox.y === box.destY && 
                     crossBox.width === box.width && crossBox.height === box.height && box.type === 'blue')
                );
                this.cancelledCache.set(i, isCancelled);
            }
    ...
    drawCrossOutBoxes() {
        if (this.model.operation === 'subtraction' && this.model.currentCellIndex >= this.model.totalCellsToAnimate) {
            for (let crossBox of this.crossOutBoxes) {
                ...
                if (pinkBox && this.model.currentCellIndex > pinkBox.animationIndex && this.model.pinkAnimationProgress === 1) {
                    this.p.stroke(255, 0, 0);
...
drawSteps() {
        const steps = this.model.stepsHistory.filter(step => step !== undefined && step !== null && typeof step === 'string');
        let y = 60;
        this.p.textSize(20);
        ...
        steps.forEach((step, index) => {
            if (!step) return;
            const x = this.p.width - 350;
            let currentX = x;
            const isCurrentStep = index === steps.length - 1;
            const textColor = isCurrentStep ? this.p.color(0, 128, 0) : this.p.color(0);
            
            let alpha = this.stepAlpha.get(step) || 255;
            if (this.model.isAnimatingEquation && this.animationStartFrame !== null && alpha < 255) {
                const frameProgress = this.p.constrain((this.p.frameCount - this.animationStartFrame) / this.animationDuration, 0, 1);
                alpha = this.p.lerp(0, 255, frameProgress);
                this.stepAlpha.set(step, alpha);
            }
            
            this.p.fill(textColor.levels[0], textColor.levels[1], textColor.levels[2], alpha);
            ...
            if (match) {
                ...
                this.p.text(step, currentX, y);
            }
            
            y += 50;
        });
        
        if (this.model.isAnimatingEquation && this.animationStartFrame !== null && this.p.frameCount - this.animationStartFrame >= this.animationDuration) {
            this.animationStartFrame = null;
            this.model.stepsHistory.forEach(step => {
                if (this.stepAlpha.get(step) < 255) {
                    this.stepAlpha.set(step, 255);
                }
            });
        }
    }
    ...
    drawFractionGrid(x, y, numerator, denominator, color, isFirstFraction = true) {
        if (this.model.sameDenominators) {
            ...
            if (this.model.currentCellIndex === 0) {
                ...
                const cols = this.model.problem.first.den;
                const rows = this.model.problem.second.den;
      ...
      setupEventListeners() {
        if (this.slider) {
            this.slider.addEventListener('input', (e) => {
                const now = Date.now();
                if (now - this.lastInputTime > 50) { // Throttle to 50ms
                    this.model.setCellIndex(parseInt(e.target.value));
                    this.lastInputTime = now;
                } else {
                    // Queue the last input
