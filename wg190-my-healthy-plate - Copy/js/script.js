...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var PINK_LINE_Y = 524;
    var METER_BOTTOM = 671.17;
    var METER_HEIGHT = 194.96;
    var YELLOW_LINE_SCALE = (METER_BOTTOM - YELLOW_LINE_Y) / METER_HEIGHT;
    var PINK_LINE_SCALE = (METER_BOTTOM - PINK_LINE_Y) / METER_HEIGHT;

    function meterScale(key, value) {
      var setting = meterSettings[key];
      var low = setting.low;
      var excess = setting.high + 1;
      if (value <= 0) return 0;
      if (value <= low) return YELLOW_LINE_SCALE * value / low;
      if (value <= excess) return YELLOW_LINE_SCALE + (PINK_LINE_SCALE - YELLOW_LINE_SCALE) * (value - low) / (excess - low);
      return PINK_LINE_SCALE + (1 - PINK_LINE_SCALE) * Math.min((value - excess) / (setting.max - excess), 1);
    }
    ...
    function updateMeter(key, value) {
      ...
      var scale = meterScale(key, value);
      var bottom = METER_BOTTOM;
      var height = METER_HEIGHT * scale;
    ...
    function updateActivity() {
      ...
      Object.keys(meterSettings).forEach(function (key) {
        ...
        var level = status === "Low" ? "low" : status === "Excess" ? "high" : "medium";
        var img = document.querySelector("#" + smileyMap[key] + " img");
        if (img) img.setAttribute("src", smileySrc[status === "Low" ? "low" : status === "Excess" ? "high" : "justRight"]);
        var capsule = document.querySelector("[data-capsule='" + key + "']");
        if (capsule) {
          capsule.dataset.targetWidth = (meterScale(key, values[key]) * 100) + "%";
          capsule.dataset.level = level;
          capsule.style.width = "0%";
          capsule.style.backgroundColor = LEVEL_COLOURS[level];
        }
      });
      updatePlateImages();
    ...
    function animateCapsules() {
      var capsules = document.querySelectorAll("[data-capsule]");
      capsules.forEach(function (cap) {
        cap.style.width = "0%";
        var target = cap.dataset.targetWidth || "0%";
        // force reflow so the transition restarts from 0 each time summary appears
        void cap.offsetWidth;
        cap.style.width = target;
      });
    }
