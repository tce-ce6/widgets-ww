...
class LottieSVGAnimation {
  ...
  constructor(
    ...
    const animType = this.jsonPath.includes("correct")
      ? "correct"
      : this.jsonPath.includes("incorrect")
      ? "wrong"
      : "default";
  ...
  init() {
    ...
    if (!path) {
      console.error(
        `No path found in ${this.containerId}. Cannot position Lottie animation.`
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        `Could not get BBox for path inside ${this.containerId}`,
        e
      );
      return;
    }
    ...
    const animName = this.jsonPath.includes("correct")
      ? "correct"
      : this.jsonPath.includes("incorrect")
      ? "incorrect"
      : "wrong-default";
    ...
    this.animation.addEventListener("DOMLoaded", () => {
      this.isReady = true;
      console.log(
        `Lottie animation loaded inside #${this.containerId} (${this.jsonPath})`
      );
    });
    ...
    this.animation.addEventListener("error", (e) => {
      console.error(
        `Lottie failed to load for #${this.containerId} (${this.jsonPath})`,
        e
      );
      this.isReady = false;
    });
  ...
  playFull() {
    if (this.isReady && this.animation) {
      ...
      console.warn(
        `Lottie is not ready or instance is missing for ${this.containerId}`
      );
      return false;
    }
  }
...
function getLeafAreaBBox() {
  ...
  const leafImg = svg ? svg.querySelector("#leaf-img") : null;
...
function parseTransform(transformStr) {
  ...
  const translateMatch = /translate\(([-\d.]+)\s*,?\s*([-\d.]+)?\)/.exec(
    transformStr
  );
  if (translateMatch) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const materialMap = {
    ...
    "translucent-material-2": {
      panel: "translucent-panel",
      category: "translucent",
    },
    "translucent-material-3": {
      panel: "translucent-panel",
      category: "translucent",
    },
  ...
  Object.keys(materialMap).forEach((materialId) => {
    ...
    if (material) {
      ...
      const startDrag = function (e) {
        ...
        if (leafGroupCheck) {
          const materialsInLeaf = Array.from(leafGroupCheck.children).filter(
            (child) =>
  ...
  function onDragMove(e) {
    ...
    try {
      ...
      console.error(
        "Failed to get fresh CTM inverse on drag move. Stopping drag.",
        error
      );
  ...
  function onDragEnd(e) {
    ...
    if (!svg) {
      ...
      try {
        ...
        console.error(
          "Failed to get fresh CTM inverse on drag end. Snapping back.",
          error
        );
      ...
      if (mapping && !droppedOnLeaf) {
        // ✅ Only allow dropping in boxes if the material was previously dropped on the leaf area
        const wasDroppedOnLeaf = materialsOnLeaf.has(currentMaterial);

        if (!wasDroppedOnLeaf) {
          console.log(
            "Material must first be dropped on the leaf area before placing into a box."
          );
          ...
          for (const boxId of allBoxIds) {
            ...
            const existingMaterialGroup = box.querySelector(
              "g.material-container"
            );
            // Check if the box is empty (no material group, or the material group is hidden)
            const targetBoxIsMaterialContainer =
              box.id.includes("-box-") || box.id.includes("-panel-");
            const isEmptyBox =
              targetBoxIsMaterialContainer &&
              (!existingMaterialGroup ||
                existingMaterialGroup.style.display === "none");
          ...
          if (targetBox) {
            ...
            let boxCategory = "";
            if (
              targetBox.id.includes("transparent-box-") ||
              targetBox.id.includes("translucent-box-")
            ) {
            ...
            const existingMaterialGroup = targetBox.querySelector(
              "g.material-container"
            );
            const targetIsEmpty =
              !existingMaterialGroup ||
            ...
            if (materialCategory === boxCategory && targetIsEmpty) {
              ...
              if (animPlayer) {
                if (!animPlayer.playFull()) {
                  console.error(
                    `Failed to play CORRECT Lottie for: ${targetBox.id}. Check Lottie file path and readiness.`
                  );
                }
              } else {
                console.error(
                  `Lottie animation instance not found for: ${targetBox.id}`
                );
              }
              ...
              if (sourceUse) {
                if (!materialGroup) {
                  ...
                  materialGroup = document.createElementNS(
                    "http://www.w3.org/2000/svg",
                    "g"
                  );
                ...
                targetUse.setAttribute(
                  "width",
                  sourceUse.getAttribute("width")
                );
                targetUse.setAttribute(
                  "height",
                  sourceUse.getAttribute("height")
                );
              ...
              if (animPlayer) {
                if (!animPlayer.playFull()) {
                  console.error(
                    `Failed to play WRONG Lottie for: ${targetBox.id}. Check Lottie file path and readiness.`
                  );
                }
              } else {
                console.error(
      ...
      if (!dropped) {
        ...
        if (
          materialsOnLeaf.has(currentMaterial) &&
          leafMaterialPositions.has(currentMaterial)
        ) {
          const leafGroup = getSVGElementById("leaf-svg");
          if (leafGroup) {
            leafGroup.appendChild(draggingMaterial);
            draggingMaterial.setAttribute(
              "transform",
              leafMaterialPositions.get(currentMaterial)
            );
          }
