document.addEventListener("DOMContentLoaded", () => {
  ...
  const teamColors = {
    ...
    oo_short: { bg: '#F5DEB3', border: '#D2691E', glow: '#CD853F', light: '#FFF8DC' },
  ...
  const soundLabel = t => `the <strong style="color:${teamColors[t].glow};font-size:20px">${vowelTeams[t].sound}</strong> sound`;
  ...
  Object.entries(vowelTeams).forEach(([teamKey, team]) => {
    ...
    team.words.forEach(word => {
      soundMap[teamKey][word.word.toLowerCase()] = soundFolders[teamKey] + word.word.toLowerCase() + '.mp3';
    });
  });
  ...
  function speak(text, team = null) {
    ...
    u.lang = 'en-IN';
    u.rate = 0.8;
    const v = speechSynthesis.getVoices().find(v => v.lang === 'en-IN' || v.lang.startsWith('en-IN'));
    if (v) u.voice = v;
  ...
  function hl(word, team, fs = '36px') {
    ...
    if (i === -1) return `<span style="font-size:${fs};font-weight:bold">${word}</span>`;
  ...
  function rb(tint) {
    return bubbles.map(b => {
      const bg = tint ? `radial-gradient(circle at 30% 30%,white,${tint})` : 'radial-gradient(circle at 30% 30%,rgba(255,255,255,0.8),rgba(200,200,255,0.3))';
  ...
  function getSteps(team) {
    ...
    if (vowelTeams[team].partner !== null) s.push({ key: 'compare', icon: '⚖️', label: 'Compare' });
  ...
  function jp(team, phase, counter) {
    ...
    h += `</div>`;
    if (counter) h += `<div class="journey-counter" style="color:${c.glow}">${counter}</div>`;
    h += `</div>`;
    return h;
  }
  ...
  function renderBubbles(containerId) {
    const container = document.getElementById(containerId);
    if (container) container.innerHTML = rb(state.currentTeam ? teamColors[state.currentTeam].bg : null);
  }
  ...
  function nextPhase() {
    ...
    if (state.phase === 'meet') {
      ...
      if (p && (state.completedTeams.includes(p) || state.completedTeams.includes(tm))) {
        genCompare();
        state.phase = 'compare';
        state.currentWordIndex = 0;
        state.showResult = null;
  ...
  function genSpot() {
    const tm = state.currentTeam;
    const tw = vowelTeams[tm].words.slice(0, 4).map(w => ({ word: w.word, hasTeam: true }));
    const d = vowelTeams[tm].distractors.slice(0, 3).map(w => ({ word: w, hasTeam: false }));
  ...
  function genCompare() {
    ...
    if (!p) return;
    const qs = [];
    vowelTeams[tm].words.slice(0, 3).forEach(w => qs.push({ word: w.word, clue: w.clue, correctTeam: tm }));
    vowelTeams[p].words.slice(0, 3).forEach(w => qs.push({ word: w.word, clue: w.clue, correctTeam: p }));
  ...
  function renderHome() {
    ...
    const islands = Object.keys(vowelTeams).map(t => {
  ...
  function updateMeet() {
    ...
    if (wi === 0) {
      ...
      html = `<p style="font-size:14px;color:#666;margin-bottom:10px">Word ${wi} of 3</p>
              ...
              <button onclick="window.vowelTwinsApp.nextMeetWord()" class="btn-main" style="margin-top:20px;background:linear-gradient(135deg,${c.border} 0%,${c.glow} 100%)">${wi < 3 ? 'NEXT →' : 'BUILD →'}</button>`;
    }
  ...
  function nextMeetWord() {
    ...
    if (state.currentWordIndex < 3) {
      state.currentWordIndex++;
      if (state.currentWordIndex > 0) setTimeout(() => speak(words[state.currentWordIndex - 1].word, tm), 300);
      updateMeet();
    } else {
      nextPhase();
    }
  ...
  function updateBuild() {
    ...
    const brc = sr === 'correct' ? '#32CD32' : sr === 'wrong' ? '#FF6B6B' : 'transparent';
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (i < parts.length - 1) hh += `<span style="display:inline-block;min-width:60px;border-bottom:4px solid ${bc};margin:0 5px;text-align:center;color:${bc}">${bv}</span>`;
    }
    
    const altTeam = vowelTeams[tm].partner || vowelTeams[tm].buildAlt || Object.keys(vowelTeams).find(k => k !== tm);
    ...
    if (!sr) {
      oh = `<p style="font-size:13px;color:#888;margin-bottom:10px">Which vowel team completes this word?</p>
            <div style="display:flex;gap:12px;flex-wrap:wrap;justify-content:center">${opts.map(t => {
      ...
      renderButtons('build-buttons', [
        { id: 'btn-check-build', label: 'CHECK', onclick: 'window.vowelTwinsApp.checkBuild()', color: state.buildAnswer ? `linear-gradient(135deg, ${c.border}, ${c.glow})` : '#DDD', borderColor: c.glow, disabled: !state.buildAnswer }
      ]);
    } else {
      ...
      renderButtons('build-buttons', [
        { id: 'btn-next-build', label: state.currentWordIndex < 4 ? 'NEXT →' : 'SPOT →', onclick: 'window.vowelTwinsApp.nextQuestion()', color: `linear-gradient(135deg, ${c.border}, ${c.glow})`, borderColor: c.glow }
      ]);
    }
  ...
  function updateSpot() {
    ...
    if (!sr) {
      ...
      renderButtons('spot-buttons', [
        { id: 'btn-next-spot', label: il ? (vowelTeams[tm].partner ? 'COMPARE →' : '⭐ Finish') : 'NEXT →', onclick: 'window.vowelTwinsApp.nextQuestion()', color: `linear-gradient(135deg, ${c.border}, ${c.glow})`, borderColor: c.glow }
      ]);
    }
  ...
  function updateCompare() {
    ...
    let body = '';
    if (!sr) {
      body = `<div style="display:flex;gap:15px">${[tm, partner].map(t => {
        const tc = teamColors[t];
      ...
      renderButtons('compare-buttons', [
        { id: 'btn-hear-compare', label: '🔊 Hear', onclick: `window.vowelTwinsApp.speak('${q.word}', '${state.compareWords[state.currentWordIndex]?.correctTeam}')`, color: '#E0E0E0', borderColor: '#BDBDBD' }
      ]);
    } else {
      ...
      renderButtons('compare-buttons', [
        { id: 'btn-next-compare', label: il ? '⭐ Finish' : 'NEXT →', onclick: 'window.vowelTwinsApp.nextQuestion()', color: 'linear-gradient(135deg, #9370DB, #7B68EE)', borderColor: '#9370DB' }
      ]);
    }
  ...
  function render() {
    const screens = ['screen-home', 'screen-meet', 'screen-build', 'screen-spot', 'screen-compare', 'screen-complete'];
    screens.forEach(s => document.getElementById(s).style.display = 'none');
    
    if (state.phase === 'home') {
      document.getElementById('screen-home').style.display = 'flex';
      document.getElementById('screen-home').style.background = 'linear-gradient(180deg,#E8F4FF 0%,#F0E6FF 50%,#FFE6F0 100%)';
      renderHome();
  ...
  window.vowelTwinsApp = {
    ...
    speak,
    nextMeetWord,
    selectBuild: (t) => { state.buildAnswer = t; updateBuild(); },
    nextWordMeet: () => { state.currentWordIndex = 1; updateMeet(); }
  };
